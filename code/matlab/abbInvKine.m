...
function [th1, th2, th3, th4, th5, th6, reachable] = abbInvKine(T_des, th_last)
    ...
    % Loop through Theta 1
    for i = 1:2
        %--- Theta 2 and Theta 3
        % Indices are defined such that:
        %   Theta2/3(1 or 3) = Theta2/3(2*i-1)
        %   Theta2/3(2 or 4) = Theta2/3(2*i)
        T0_1 = dhTransform(linkList(1).a, linkList(1).d, linkList(1).alpha, ...
                       theta1_real(i,1));
        R0_1 = T0_1(1:3,1:3);
        d1_15 = (R0_1')*(d0_06 - z0_6*linkList(6).d - [0;0;1]*linkList(1).d);
        % Theta3_1, Theta2_1
        theta3_tmp((2*i-1),1) = pi + 2*atan(sqrt((2*linkList(2).a*sqrt(linkList(4).d^2 + ...
        ...
        theta3_real((2*i-1),1) = real(theta3_tmp((2*i-1),1));
        zeta = (linkList(2).a + linkList(3).a*cos(theta3_real((2*i-1),1)) + linkList(4).d* ...
            cos(theta3_real((2*i-1),1) + pi/2));
        beta = (linkList(3).a*sin(theta3_real((2*i-1),1)) + linkList(4).d*sin(theta3_real((2*i-1),1) + ...
            pi/2));
        theta2_tmp((2*i-1),1) = atan2((zeta*d1_15(2,1) - beta*d1_15(1,1))/(zeta^2 + beta^2), ...
            (zeta*d1_15(1,1) + beta*d1_15(2,1))/(zeta^2 + beta^2));
        theta2_real((2*i-1),1) = real(theta2_tmp((2*i-1),1));
        % Theta3_2, Theta2_2
        theta3_tmp((2*i),1) = pi - 2*atan(sqrt((2*linkList(2).a*sqrt(linkList(4).d^2 + ...
            linkList(3).a^2) - linkList(2).a^2 - linkList(4).d^2 - linkList(3).a^2 + ...
            norm(d1_15)^2)/(2*linkList(2).a*sqrt(linkList(4).d^2 + linkList(3).a^2) + ...
            linkList(2).a^2 + linkList(4).d^2 + linkList(3).a^2 - norm(d1_15)^2))) - ...
            atan2(linkList(4).d, linkList(3).a);
        theta3_real((2*i),1) = real(theta3_tmp((2*i),1));
        zeta = (linkList(2).a + linkList(3).a*cos(theta3_real((2*i),1)) + linkList(4).d* ...
            cos(theta3_real((2*i),1) + pi/2));
        beta = (linkList(3).a*sin(theta3_real((2*i),1)) + linkList(4).d*sin(theta3_real((2*i),1) + ...
            pi/2));
        theta2_tmp((2*i),1) = atan2((zeta*d1_15(2,1) - beta*d1_15(1,1))/(zeta^2 + beta^2), ...
            (zeta*d1_15(1,1) + beta*d1_15(2,1))/(zeta^2 + beta^2));
        theta2_real((2*i),1) = real(theta2_tmp((2*i),1));
        
        % Loop through Theta 2 and 3
        for j = 1:2
            %--- Theta 4, 5 and 6
            % Indices are defined such that:
            %   Theta4/5/6(1 or 3 or 5 or 7) = Theta2/3(4*(i-1) + (2*j-1))
            %   Theta4/5/6(2 or 4 or 6 or 8) = Theta2/3(4*(i-1) + (2*j))
            T0_3 = dhFwdKine(linkList(1:3), [theta1_real(i,1);theta2_real(2*(i-1)+j,1);theta3_real(2*(i-1)+j,1)]);
            R0_3 = T0_3(1:3,1:3);
            R3_6 = (R0_3')*T_des(1:3,1:3);
            theta5_tmp(4*(i-1)+(2*j-1),1) = atan2(sqrt(R3_6(3,1)^2 + R3_6(3,2)^2), R3_6(3,3));
            theta5_tmp(4*(i-1)+(2*j),1) = atan2(-sqrt(R3_6(3,1)^2 + R3_6(3,2)^2), R3_6(3,3));
            theta5_real(4*(i-1)+(2*j-1),1) = real(theta5_tmp(4*(i-1)+(2*j-1),1));
            theta5_real(4*(i-1)+(2*j),1) = real(theta5_tmp(4*(i-1)+(2*j),1));
            if ((abs(wrapToPi(theta5_real(4*(i-1)+(2*j-1),1))) < tol) && (abs(wrapToPi(theta5_real(4*(i-1)+(2*j-1),1))) > -tol))
                theta4_tmp((4*(i-1)+(2*j-1)):(4*(i-1)+(2*j)),1) = zeros(2,1);
                theta6_tmp((4*(i-1)+(2*j-1)):(4*(i-1)+(2*j)),1) = ones(2,1)*atan2(R3_6(2,1), R3_6(1,1));
            elseif (abs(wrapToPi((theta5_real(4*(i-1)+(2*j-1),1))) < (pi+tol)) && (abs(wrapToPi(theta5_real(4*(i-1)+(2*j-1),1))) > (pi-tol)))
                theta4_tmp((4*(i-1)+(2*j-1)):(4*(i-1)+(2*j)),1) = zeros(2,1);
                theta6_tmp((4*(i-1)+(2*j-1)):(4*(i-1)+(2*j)),1) = -1*ones(2,1)*atan2(-R3_6(2,1), -R3_6(1,1));
            else
                theta4_tmp(4*(i-1)+(2*j-1),1) = atan2(-R3_6(2,3)/sin(theta5_real(4*(i-1)+(2*j-1),1)), -R3_6(1,3)/sin(theta5_real(4*(i-1)+(2*j-1),1)));
                theta4_real(4*(i-1)+(2*j-1),1) = real(theta4_tmp(4*(i-1)+(2*j-1),1));
                theta6_tmp(4*(i-1)+(2*j-1),1) = atan2(-R3_6(3,2)/sin(theta5_real(4*(i-1)+(2*j-1),1)), R3_6(3,1)/sin(theta5_real(4*(i-1)+(2*j-1),1)));
                theta6_real(4*(i-1)+(2*j-1),1) = real(theta6_tmp(4*(i-1)+(2*j-1),1));
                theta4_tmp(4*(i-1)+(2*j),1) = atan2(-R3_6(2,3)/sin(theta5_real(4*(i-1)+(2*j),1)), -R3_6(1,3)/sin(theta5_real(4*(i-1)+(2*j),1)));
                theta4_real(4*(i-1)+(2*j),1) = real(theta4_tmp(4*(i-1)+(2*j),1));
                theta6_tmp(4*(i-1)+(2*j),1) = atan2(-R3_6(3,2)/sin(theta5_real(4*(i-1)+(2*j),1)), R3_6(3,1)/sin(theta5_real(4*(i-1)+(2*j),1)));
                theta6_real(4*(i-1)+(2*j),1) = real(theta6_tmp(4*(i-1)+(2*j),1));
            end
        end
    end
    ...
    % table
    solutions(:,2) = wrapToPi(solutions(:,2) + pi/2);
    th2 = solutions(:,2);
    th3 = solutions(:,3);
    th4 = solutions(:,4);
    th5 = solutions(:,5);
    ...
    if (exist('th_last', 'var'))
        ...
        % Find wrapToPi offset
        th_offset = th_last - wrapToPi(th_last);
        th_last = wrapToPi(th_last);
        
        % Performs least squares solution for th4 and th6 when th5 is
        % degenerate
        for i = 1:8
            if ((th5(i,1) < tol) && (th5(i,1) > -tol))
                % Find sum closes to th_last(4) + th_last(6) by adding or
                % subtracting 2*pi
                theta_sum = solutions(i,6);
                th_last_sum = th_last(4) + th_last(6);

                % check current error
                error_cur = theta_sum - th_last_sum;
                error_up = (theta_sum + 2*pi) - th_last_sum;
                error_down = (theta_sum - 2*pi) - th_last_sum;
                if (error_up^2 < error_cur^2)
                    theta_sum = theta_sum + 2*pi;
                elseif (error_down^2 < error_cur^2)
                    theta_sum = theta_sum - 2*pi;
                end
                    
                solutions(i,4) =  th_last(4)/2 - th_last(6)/2 + theta_sum/2;
                solutions(i,6) = -th_last(4)/2 + th_last(6)/2 + theta_sum/2;
            elseif (abs(wrapToPi((th5(i,1))) < (pi+tol)) && (abs(wrapToPi(th5(i,1))) > (pi-tol)))
                theta_diff = solutions(i,6);
                solutions(i,6) =  th_last(4)/2 + th_last(6)/2 + theta_diff/2;
                solutions(i,4) =  solutions(i,6) - theta_diff;
            end
        end
        ...
        [~, min_index] = min(error_sq);
        % Add offset to solution
        solutions(min_index,:) = solutions(min_index,:) + th_offset';
        th1 = solutions(min_index, 1);
        th2 = solutions(min_index, 2);
